...
void solve() {
    ...
    long long count=0;
    for(int i=0; i<n; i++){
        auto it = lower_bound(ends.begin(), ends.end(), intervals[i].first);//first element >= i
        
            
        //cout<<mp[intervals[i].second]<<endl;
        bit.add(mp[intervals[i].second]+1,1);
        //cout<<distance(ends.begin(),it)<<endl;
        count+=(i-bit.query(distance(ends.begin(),it)));
